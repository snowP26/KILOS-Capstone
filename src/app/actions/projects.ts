...
const uploadFile = async (
  ...
  const { error: bucketError } = await client.storage
    .from("projects")
    .upload(`files/${location}/${filename}`, file, { upsert: true });
...
export const getProjectBudgetById = async (
  project_id: number,
  isHome?: boolean
) => {
  let query = client
...
export const addBudget = async (
  ...
  if (receipt && receipt.size > 0) {
    ...
    const { error } = await client.storage
      .from("projects")
      .upload(`photos/${loc}/${project}/receipts/${filename}`, receipt, {
        upsert: false,
      });
  ...
  if (item_photo && item_photo.size > 0) {
    ...
    const { error } = await client.storage
      .from("projects")
      .upload(`photos/${loc}/${project}/items/${filename}`, item_photo, {
        upsert: false,
      });
  ...
  const { error } = await client.from("project_budget").insert([
    {
      project_id: projectID,
      status: "For Approval",
      item_name: item,
...
export const deletePhoto = async (itemID: number, item: boolean) => {
  ...
  const filePath = photoURL?.includes(basePath)
    ? photoURL.split(basePath)[1]
    : photoURL;
...
export const updateBudgetStatus = async (
  ...
  console.log(
    `Updated budgetID ${budgetID} to ${status} ${comment ? "with comment" : ""}`
  );
  return true;
};
...
export const updateBudget = async (projectID: number, newBudget: number) => {
  ...
  try {
    ...
    Swal.fire({
      ...
      text: `You have successfully updated the project's budget to ${newBudget.toLocaleString(
        "en-PH",
        {
          style: "currency",
          currency: "PHP",
...
export const fetchNotifs = async (projectID: number) => {
  try {
    const { data, error } = await client
      .from("project_notifications")
      .select("*")
    ...
    Swal.fire({
      ...
      text: `Unable to load project notifications. ${error.message}`,
      icon: "error",
      timer: 1500,
    });
...
export const uploadCSVItems = async (
