    ...
    if (updatedData) {
      const processed = await Promise.all(
        updatedData.map(async (item) => {
          let publicUrl = null;
          if (item.photo) {
            publicUrl = await getPhoto(item.photo);
          }
          ...
          return {
            ...
            authorPosition: authorInfo.position ?? "N/A",
            authorRole: authorInfo.role ?? "N/A",
          };
        })
      );
    ...
    <div className="h-full bg-[#E6F1FF]">
      <div className="flex flex-row">
        ...
        <div className="w-3/5 my-2 mx-3">
          ...
          {announcements
            ...
            .map((data) => (
              <div
                key={data.id}
                className="bg-white rounded-2xl w-[100%] pl-10 pr-10 pb-2 mb-2"
              >
                <div className="flex flex-row justify-between">
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-row gap-5">
                      ...
                      <p className="text-sm font-thin">{data.authorPosition} • {data.authorRole}</p>
                ...
                <div className="mt-7 mb-10 flex flex-row justify-between">
                  <div className="flex flex-col">
                    ...
                    <p className="w-150">{data.body}</p>
                  </div>
                  {data.photo && (
                    <img
                      alt="Announcement image"
                      className="bg-black max-w-50 max-h-50"
                      src={data.photo}
                    />
                  )}
                </div>
        ...
        <div className="w-1/5 mr-3">
          <Button
            className="bg-[#052659] w-[100%] my-3 hover:bg-accent hover:text-accent-foreground hover:border-accent-foreground cursor-pointer hover:border-1"
            onClick={() =>
